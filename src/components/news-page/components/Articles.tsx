                ...
                for(let a in data.articles)
                    elements.push(
                        <div id="article-entry" key={data.articles[a].title}>
                            <a className="article-link" href={`/aktualnosci/${data.articles[a].title.replace(' ', '+').toLowerCase()}`}>
                                {(data.articles[a].thumbnail !== null) ? 
                                    ...
                                    <div className="article-text">
                                        <div className="article-title">{(data.articles[a].title.length > 20) ? `${data.articles[a].title.substring(0, 25)}...` : data.articles[a].title}</div>
